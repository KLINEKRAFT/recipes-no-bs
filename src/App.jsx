...
// --- API calls go through Cloudflare Function ---
...
async function extractRecipe(url) {
  ...
  const data = await res.json();
  if (data.error) throw new Error(data.error);
  if (!data.result) throw new Error("Could not extract recipe");
  return data.result;
}
...
async function fetchRotd() {
  const res = await fetch("/api/recipes", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "rotd" }),
  });
  const data = await res.json();
  if (data.error) throw new Error(data.error);
  if (!data.result) throw new Error("No recipe found");
  return data.result;
}
...
function Img({ src, title, h, style }) {
  ...
  if (!src || bad) {
    ...
    const icon = iconKeys[hash(title) % iconKeys.length];
    return <div style={{ width: "100%", height: h, background: bg, display: "flex", alignItems: "center", justifyContent: "center", ...style }}>{ICONS[icon]("#ccc", 32)}</div>;
  }
...
function BrowseCard({ recipe, index, onClick }) {
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ fontFamily: "var(--serif)", fontSize: 19, color: "#111", lineHeight: 1.25, marginBottom: 4, letterSpacing: "-0.01em" }}>{recipe.title}</div>
      {recipe.description && (
        <div style={{ fontFamily: "var(--sans)", fontSize: 13, color: "#888", lineHeight: 1.45, marginBottom: 6, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{recipe.description}</div>
      )}
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()}
      ...
      <div style={{ maxWidth: 620, margin: "0 auto", padding: "40px 28px 60px", position: "relative" }}>
        ...
        <p style={{ fontFamily: "var(--mono)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.12em", color: ACC, marginBottom: 16 }}>{recipe.source || "Recipe"}</p>
          ...
          <>
            ...
            {data.tips && (
              <div style={{ marginTop: 32, paddingTop: 24, borderTop: "1px solid #eee" }}>
                <p style={{ fontFamily: "var(--mono)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: ACC, marginBottom: 10 }}>Notes</p>
    ...
    <div style={{ minHeight: "100vh", background: "#fff", color: "#111" }}>
      ...
      <header style={{ padding: "20px 32px", borderBottom: "1px solid #f0f0f0", display: "flex", alignItems: "baseline", justifyContent: "space-between", flexWrap: "wrap", gap: 16, position: "sticky", top: 0, zIndex: 100, background: "rgba(255,255,255,0.97)", backdropFilter: "blur(10px)" }}>
        <div onClick={() => setView("home")} style={{ cursor: "pointer" }}>
          <span style={{ fontFamily: "var(--serif)", fontSize: 26, fontWeight: 300, color: "#111", letterSpacing: "-0.03em" }}>Recipes with No Bullshit</span>
        </div>
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <input value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === "Enter" && doUrl()} placeholder="Paste any URL" style={{ border: "none", borderBottom: "1px solid #e5e5e5", padding: "6px 0", fontSize: 14, fontFamily: "var(--sans)", width: 160, color: "#111", outline: "none", background: "transparent" }} />
        ...
        <main style={{ maxWidth: 800, margin: "0 auto", padding: "56px 32px 80px" }}>
          <section style={{ marginBottom: 64 }}>
            <h1 style={{ fontFamily: "var(--serif)", fontSize: 52, fontWeight: 300, color: "#111", lineHeight: 1.05, letterSpacing: "-0.03em", marginBottom: 12 }}>
              Recipes,<br />nothing else<span style={{ color: ACC }}>.</span>
            </h1>
          ...
          <section style={{ marginBottom: 64 }}>
              ...
              <div onClick={doRotd} style={{ cursor: "pointer", paddingBottom: 32, borderBottom: "1px solid #f0f0f0" }}>
                <div style={{ width: "100%", height: 320, background: "#f8f6f3", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{ fontFamily: "var(--mono)", fontSize: 10, color: "#ddd", textTransform: "uppercase", letterSpacing: "0.1em" }}>A seasonal recipe, stripped clean</div>
                  </div>
                </div>
              </div>
            )}
            ...
            {rotd && !rotdLoad && (
              <div onClick={() => open(rotd)} style={{ cursor: "pointer", animation: "fadeUp 0.5s ease", paddingBottom: 32, borderBottom: "1px solid #f0f0f0" }}>
                ...
                {rotd.tagline && <p style={{ fontFamily: "var(--mono)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.1em", color: "#bbb", marginBottom: 8 }}>{rotd.tagline}</p>}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
              {CATEGORIES.map((cat, i) => (
                <div key={cat.id} onClick={() => load(cat.query, cat.id)}
                  style={{ padding: "18px 4px", cursor: "pointer", borderBottom: "1px solid #f5f5f5", display: "flex", alignItems: "center", gap: 14, transition: "padding-left 0.2s", animation: `fadeUp 0.3s ease ${i * 0.03}s both` }}
      ...
      {view === "browse" && (
        <main style={{ maxWidth: 900, margin: "0 auto", padding: "32px 32px 80px" }}>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 32, paddingBottom: 20, borderBottom: "1px solid #f0f0f0", alignItems: "center" }}>
            <span onClick={() => setView("home")} style={{ fontFamily: "var(--mono)", fontSize: 10, color: "#bbb", cursor: "pointer", textTransform: "uppercase", letterSpacing: "0.08em", marginRight: 8 }}>Home /</span>
            {CATEGORIES.map(cat => (
              <button key={cat.id} onClick={() => load(cat.query, cat.id)} style={{
                ...
                border: "none", padding: "4px 12px", fontFamily: "var(--mono)", fontSize: 10,
